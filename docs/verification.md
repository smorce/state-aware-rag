...
| `textSearchNodesWith("Chunk", "body", ...)` | `"State"` / `"RAG"` / `"サンプル"` などスペース区切り英数語は hit。Phase 2 以降は query と Helix 側 `Chunk.body` を `normalize_for_fulltext` で同じスペース区切り表現へ正規化するため、日本語 query も再 ingest 済み文書では hit 対象になる。 |
...
Phase 2 contract test では、次の仕様ギャップ解消を確認済み。

- `HelixBackedRagStore.update_working_memory` が `status` / `round_count` / `updated_at` を Helix `WorkingMemory` へ `SetProperty` 同期する。
- Evidence はあるが active `MemoryNote` がない場合、最終回答は §15.2 固定メッセージになる。
- `SearchRound -RETURNED-> Evidence` は `RoundLog.accepted_evidence_ids` の ID だけを使い、過去ラウンドの Evidence を末尾推定しない。
- graph search は `CONFLICTS_WITH` 起点の関連 Chunk を含める。
- Helix ingest 時の `Chunk.body` は全文検索用に正規化し、SQLite mirror は原文を保持する。
- オーケストレータ内の未処理例外では `WorkingMemoryStatus.FAILED` を保存してから例外を再送出する。
...
(:Evidence {working_memory_id})-[:FROM_CHUNK]->(:Chunk)<-[:HAS_CHUNK]-(:Document)-[:HAS_CHUNK]->(:Chunk)
(:WorkingMemory)-[:HAS_NOTE]->(:MemoryNote)-[:CONFLICTS_WITH]->(:MemoryNote)-[:RELATED_TO]->(:Entity)<-[:MENTIONS]-(:Chunk)
```

Document 近傍は Helix から取得した sibling Chunk を Python 側で `abs(position差) <= 1` に絞る。SQLite mirror は候補発見には使わず、返却候補の表示用原文 `body` と型復元に使う。
