...
```powershell
uv run state-aware-rag --db tmp_flow.sqlite3 ingest tmp_flow_sample.md
uv run state-aware-rag --db tmp_flow.sqlite3 ask "What should the final answer use?" --llm server --bosun rule --max-rounds 1
```
