...
- [ ] Phase 0: Helix DSL spike（traversal パターン確定）
- [ ] Phase 1: Evidence に `working_memory_id` を Helix 同期
- [ ] Phase 2: `helix_neighbor_chunks_for_evidence` 実装
- [ ] Phase 3: `helix_graph_search` 統合・Document 近傍の mirror 除去
- [ ] Phase 4: テスト更新（Document 近傍）
- [ ] Phase 5: ドキュメント更新（中間）
- [ ] Phase 6: 矛盾 note 経由 Entity 展開の Helix 化・`mirror_sources` 完全除去
